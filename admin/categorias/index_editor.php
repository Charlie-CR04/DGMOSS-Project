<?php
    ...
    // Obtener la dirección del editor
    $id_direccion = (int)($_SESSION['id_direccion'] ?? 0);  // Dirección del editor desde la sesión
    $limit = 10;
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $offset = ($page - 1) * $limit;

    // Consulta para obtener las categorías con la paginación, filtrando por dirección
    ...
    // Obtener el total de categorías para calcular el número de páginas, filtrado por dirección
    $total_categorias_sql = "SELECT COUNT(DISTINCT c.id_categoria) as total 
                             FROM categorias c
                             LEFT JOIN documentos d ON d.id_categoria = c.id_categoria
                             LEFT JOIN direcciones dir ON dir.id_direccion = d.id_direccion
                             WHERE dir.id_direccion = ?";
